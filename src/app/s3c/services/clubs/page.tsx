    ...
    <>
      ...
      <main className="w-full max-w-5xl mx-auto px-4 md:px-6 py-8 md:py-12 pb-36">
        <div className="mb-8 md:mb-10 text-center md:text-left">
          <h1 className="text-xl md:text-4xl font-bold text-[#0F172A] mb-2 tracking-tight">NUST Clubs & Societies</h1>
          <p className="text-sm md:text-lg text-[#64748B]">Discover and join campus organizations.</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {clubs.map((club) => {
            ...
            return (
              <div key={club.id} className="bento-card-light p-4 md:p-6 flex flex-col sm:flex-row items-center sm:items-start gap-4 md:gap-6 hover:border-[#F97316] transition-colors group">
                <div className={`w-14 h-14 md:w-16 md:h-16 rounded-full ${club.bg} flex items-center justify-center shrink-0 shadow-sm`}>
                  <Icon size={28} className={`${club.color} md:w-8 md:h-8 group-hover:scale-110 transition-transform`} strokeWidth={2} />
                </div>
                <div className="flex-1 text-center sm:text-left">
                  <span className="text-[10px] font-bold text-[#64748B] uppercase tracking-widest">{club.category}</span>
                  <h2 className="text-lg md:text-xl font-bold text-[#0F172A] mb-1.5 md:mb-2">{club.name}</h2>
                  <p className="text-[13px] md:text-sm text-[#475569] mb-4 leading-relaxed">{club.description}</p>
                  <button 
                    onClick={() => setSelectedClub(club)}
                    className="w-full sm:w-auto bg-[#F8FAFC] text-[#F97316] border border-[#E2E8F0] hover:border-[#F97316] hover:bg-[#FFF7ED] font-bold py-2 px-6 rounded-lg transition-colors text-sm"
                  >
                    Apply Now
                  </button>
                </div>
