...
export default async function LMSSubmitPage({ searchParams }: { searchParams: Promise<{ title?: string, course?: string, date?: string }> }) {
  const resolvedParams = await searchParams;
  const title = resolvedParams?.title || "Data Structures Assignment 3";
  const course = resolvedParams?.course || "CS-201";
  const date = resolvedParams?.date || "Tonight, 11:59 PM";
      ...
      <main className="w-full max-w-4xl mx-auto px-6 py-12 pb-32 flex flex-col gap-6">
        ...
        <div className="bento-card flex flex-col gap-6 bg-white">
          <div className="flex items-start gap-3 p-4 bg-[#E4E613]/10 border border-[#E4E613] rounded-[12px]">
            ...
            <div>
              <p className="text-sm font-semibold text-[#171717]">Deadline: {date}</p>
